...
#[inline]
pub fn do_sin_coeefs(d: f32, coeefs: &Vec<f32>) -> f32 {
    let qf = rintfk(std::f32::consts::FRAC_1_PI * d);
    let q = qf as i32;
    let mut r = mlaf(qf, -PI_A_F, d);
    r = mlaf(qf, -PI_B_F, r);
    ...
    let mut u = coeefs[4];
    u = mlaf(u, x2, coeefs[3]);
    u = mlaf(u, x2, coeefs[2]);
    u = mlaf(u, x2, coeefs[1]);
    u = mlaf(u, x2, coeefs[0]);
    u = mlaf(u, x2 * r, r);
    if u.isnegzero() {
        return 0f32;
    }
