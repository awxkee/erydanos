...
use crate::{
    _mm_abs_ps, _mm_copysign_ps, _mm_exp_ps, _mm_expq_fast_ps, _mm_isinf_ps, _mm_isnan_ps,
    _mm_isneginf_ps, _mm_ln_fast_ps, _mm_ln_ps, _mm_select_ps,
};
...
pub unsafe fn _mm_pow_ps(d: __m128, n: __m128) -> __m128 {
    ...
    let is_any_nan = _mm_or_ps(_mm_isnan_ps(d), _mm_isnan_ps(n));
    let mut ret = _mm_select_ps(is_infinity, _mm_set1_ps(f32::INFINITY), c);
...
pub unsafe fn _mm_pow_fast_ps(d: __m128, n: __m128) -> __m128 {
    let mut c = _mm_expq_fast_ps(_mm_mul_ps(n, _mm_ln_fast_ps(d)));
    c = _mm_copysign_ps(c, d);
    c
}
...
mod tests {
    ...
    fn test_powf() {
        ...
        unsafe {
            ...
            let power = _mm_set1_ps(1. / 5.);
            let comparison = _mm_pow_ps(value, power);
            let flag_1 = f32::from_bits(_mm_extract_ps::<0>(comparison) as u32).to_bits();
            // Rust returns NAN for negative values with < 1 power, this is not correct
